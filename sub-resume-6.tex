...
\begin{proof}
  ...
  $\rho_{t} \colon X \longrightarrow Y$：ホモトピー，$h \colon s \longmapsto \rho_{s}(x_{0})$：パス，とする。
  このとき，$(\rho_{0})_{*} = \beta_{h} \circ (\rho_{1})_{*}$である。
  ...
  \begin{tikzpicture}
    ...
    \draw (2.8,1) node[above=8, left=-3]{$\rho_{1}(x_{0})$} .. controls (4.5,0) and (4.3,2) .. (2.8,1);
    \filldraw (2.5,0) circle [radius=0.05];
    \draw (2.5,0) .. controls (4.2,-1) and (4,1) .. (2.5,0);
    \filldraw (2.3,-1) circle [radius=0.05];
    \draw (2.3,-1) node[below=8, left=-3]{$\rho_{0}(x_{0})$} .. controls (4,-2) and (3.8,0) .. (2.3,-1);
    \draw (2.3,-1) .. controls (2.5,0) .. (2.8,1);
    \node at (2.3,0.4) {$h$};
    \node at (4.5,1.3) {$\rho_{1} \circ f$};
    \node at (4.2,0.3) {$\rho_{t} \circ f$};
    \node at (4,-0.7) {$\rho_{0} \circ f$};
  \end{tikzpicture}

  これは次のようにして示される。：
  \begin{equation}
    (\beta_{h} \circ (\rho_{1})_{*}) ([f]) = \beta_{h} ([\rho_{1} \circ f]) = [h \cdot \rho_{1} \circ f \cdot \overline{h}] = [\rho_{0} \circ f] = (\rho_{0})_{*} ([f])
  \end{equation}
  なお，後ろから2つ目の等号はホモトピー$\sets{h_{t} \cdot \varphi_{t} \circ f \cdot \overline{h}_{t}}$による。

  \sukima \midashi{［本証明］}

  ホモトピー$\sets{\rho_{t}}$によって$\phi \circ \varphi \simeq \mathrm{id}$となるならば，あるパス$h$が存在して$\phi_{*} \circ \varphi_{*} = \beta_{h}$となる。
  特に写像$\phi_{*} \circ \varphi_{*}$は$\pi_{1}(X, x_{0})$と$\pi_{1}(X, (\phi \circ \varphi)(x_{0}))$の間の同型写像である。このことから$\varphi_{*}$は単射である。
  同様に$\varphi_{*} \circ \phi_{*}$は同型写像であり，$\phi_{*}$は単射である。よって$\varphi_{*}$は全単射である。
\end{proof}
